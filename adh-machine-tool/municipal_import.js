// adh-machine-tool/municipal_import.js - imports HALF of the real Ekurhuleni municipal account
// statement for 55 Loper Street (account 1702343124, "METBOARD PROPERTIES") - this account bills
// BOTH 55 Loper Ave - ADH Machine Tool South Africa (PTY) Ltd AND 55 Loper Ave - Zelvio Global under
// one physical municipal account (see zelvio-global/municipal_import.js for the other half, and
...
// physical building consumption alongside its assumed half-share of the Rand bill. Zelvio Global's
// own municipal_import.js uses the exact same readings and rates - the two together always sum back
// to the statement's own real, unsplit figures.
//
...
const TARIFF_NAME = 'Ekurhuleni_Municipal_Account_55 Loper Street - Shared (50% - ADH Machine Tool)';

// Full statement figures (excl. VAT), before the 50/50 split - kept here as plain comments so the
// halving below is easy to audit against the source PDF:
...
function main(dbFile = 'adh-machine-tool.db') {
  ...
  if (created) console.log(`ADH Machine Tool municipal account import: ${created} statement(s) added (Jul 2026, 50% share of the shared 55 Loper Street account).`);
  return db;
}
